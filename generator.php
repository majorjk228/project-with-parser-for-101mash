...
<table class="table table-bordered table-dark table-responsive" align="center" id="info-table"> 
  <thead>
  ...
  <tbody>
    ...
    <?php  foreach($conn->query($sql) as $row)   :?>
      ...
      <?php 
            ...
            echo '<td width="150"><button type="button" value="'. $row['id_lot'] . '" data-lot="'. $row['id_lot'] . '" class="btn btn-success like">Нравится</button><button type="button" width="70" value="'. $row['id_lot'] . '" data-lot="'. $row['id_lot'] . '"" class="btn btn-danger dislike">Не нравится</button></td>' . '<td><input type="button" id="submit" class="btn btn-info title" name="title" value="' . $row['id_lot']  . '" data-lot="'. $row['id_lot'] . '"></td>' . '<td> <p class="aligncenter"><img src="' . $row['img_lot'] . '"width="350"></p></td>'. '<td class="text">' . $row['text_lot'] . '</td>'
